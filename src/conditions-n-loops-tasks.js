...
function convertToRomanNumerals(num) {
  ...
  const numeralValues = [1, 4, 5, 9, 10];
  let result = '';
  let remainingNum = num;
  for (let i = romanNumerals.length - 1; i >= 0; i -= 1) {
...
function isContainNumber(num, digit) {
  ...
  while (remainingDigits > 0) {
    const currentDigit = remainingDigits % 10;
...
function shuffleChar(str, iterations) {
  ...
  for (let i = iterations; i > 0; i -= 1) {
    ...
    for (let k = 0; k < strCopy.length; k += 1) {
      if (k % 2 !== 0) {
        sbOdd += strCopy[k];
      } else {
        sbEven += strCopy[k];
...
function getNearestBigger(number) {
  ...
  if (i >= 0) {
    ...
    const rightOfI = digits.splice(i + 1);
    rightOfI.sort((a, b) => a - b);
    digits.push(...rightOfI);
